...
async function ensureSkippedUpstreamEvents(conn, budgetId, actor = {}) {
  /**
   * v9 fix:
   * - loadChain was referenced but never defined, causing noisy logs.
   * - Budgets can contain items with different templates, so we detect skipped stages per template.
   *
   * What we do:
   *  1) Look at the template used by each item (budget_items.route_template_id)
   *  2) Load stages for each template from workflow_template_stages
   *  3) If a template omits 'needed' and/or 'cost', insert one idempotent "skipped" event per item+stage
   */
  // normalize actor ids (support both key styles)
  ...
  // 1) Find templates used in this budget
  const [tplRows] = await conn.query(
    `SELECT DISTINCT bi.route_template_id AS template_id
       FROM budget_items bi
      WHERE bi.budget_id = ?
        AND bi.route_template_id IS NOT NULL`,
    [budgetId]
  );

  const templateIds = (tplRows || [])
    .map((r) => Number(r.template_id))
    .filter(Number.isFinite);

  if (!templateIds.length) return;

  // 2) Load stages per template
  const [stageRows] = await conn.query(
    `SELECT template_id, LOWER(TRIM(stage)) AS stage
       FROM workflow_template_stages
      WHERE template_id IN (?)`,
    [templateIds]
  );

  const stagesByTemplate = new Map(); // templateId -> Set(stageKey)
  for (const r of stageRows || []) {
    const tid = Number(r.template_id);
    const st = String(r.stage || "").toLowerCase();
    if (!Number.isFinite(tid) || !st) continue;
    if (!stagesByTemplate.has(tid)) stagesByTemplate.set(tid, new Set());
    stagesByTemplate.get(tid).add(st);
  }

  // 3) Compute which stages are skipped per template
  const skips = []; // { templateId, stage }
  for (const tid of templateIds) {
    const stageSet = stagesByTemplate.get(tid) || new Set();
    if (!stageSet.has("needed")) skips.push({ templateId: tid, stage: "needed" });
    if (!stageSet.has("cost")) skips.push({ templateId: tid, stage: "cost" });
  }
  if (!skips.length) return;

  // 4) Insert skip events per (templateId, stage) for items using that template
  async function insertSkip(templateId, stageLiteral, fallbackToNoteIfEnumMissing = true) {
    try {
      await conn.query(
        `
        INSERT INTO budget_item_events
        ...
        SELECT bi.budget_id, bi.id, ?, 'skipped', NULL, NULL,
               ...
               JSON_OBJECT('stage', ?, 'reason', 'template_skipped', 'template_id', ?),
               ?, ?
          FROM budget_items bi
         WHERE bi.budget_id = ?
           AND bi.route_template_id = ?
           AND NOT EXISTS (
      ...
      await conn.query(
        ...
        SELECT bi.budget_id, bi.id, ?, 'note', NULL, 'skipped',
               ...
               JSON_OBJECT('stage', ?, 'reason', 'template_skipped', 'fallback', true, 'template_id', ?),
               ?, ?
          FROM budget_items bi
         WHERE bi.budget_id = ?
           AND bi.route_template_id = ?
  ...
  for (const s of skips) {
    await insertSkip(s.templateId, s.stage);
  }
}
