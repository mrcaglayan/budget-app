...
/* -------------------- Status recompute (STRICT final-status rule) -------------------- */
  /*
    COMPLETE IFF every item for this budget has final_purchase_status in ('approved','adjusted','rejected').
    Otherwise keep current budget_status (do not force 'in_review').
  */
async function recomputeCoordinatorStatus(conn, budgetId, actor = {}) {
  ...
  // Optional hook; safe to skip if not present
  try {
  ...
  // Count all items vs finalized items (strict list)
  ...
  // Not all items finalized: keep current budget_status (do not force in_review).
}
...
/* -------------------- BULK Reject (marks as 'rejected') -------------------- */
router.patch(
  "/bulk-Reject",
  authenticateAndAttachPermissions,
  async (req, res, next) => {
    try {
      const { account_id, budget_id, account_dept_id, dept_label } = req.body ?? {};

      const accountId = Number(account_id);
      const budgetId = Number(budget_id);
      const deptLabel =
        typeof (account_dept_id ?? dept_label) === "string"
          ? (account_dept_id ?? dept_label).trim()
          : "";
      ...
      const reqDeptId = Number(req.user?.department_id || 0) || null;
      ...
      try {
        ...
        // Dept scoping — notes plain text OR JSON notes.dept_label
        if (deptLabel) {
          where += `
            AND (
              INSTR(bi.notes, ?) > 0
        ...
        const sql = `
          ...
          SET
            final_purchase_status   = 'rejected',
        ...
        // Correct param order: SET param first, then WHERE params, then optional deptLabel params
...
router.put(
  ...
  async (req, res) => {
    ...
    await pool.promise().query(
      `
        INSERT INTO account_postpone_note
